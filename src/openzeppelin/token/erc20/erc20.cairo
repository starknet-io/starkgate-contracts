...
mod ERC20 {
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _increase_allowance(
            ...
            let caller = get_caller_address();
            self._approve(
                caller, spender, self.ERC20_allowances.read((caller, spender))
                + added_value);
            true
        }
        ...
        // or alternatively mints (or burns) if `from` (or `to`) is
        /// the zero address. All customizations to transfers, mints,
        // and burns should be done by overriding this function.
