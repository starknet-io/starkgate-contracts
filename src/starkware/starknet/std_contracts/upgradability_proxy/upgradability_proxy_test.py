...
@pytest.fixture
def starknet(session_starknet: Starknet) -> Starknet:
...
@pytest.fixture
def wrapped_impl(starknet: Starknet, proxy_contract: StarknetContract) -> StarknetContract:
...
async def test_block_init(proxy_contract, wrapped_impl) -> None:
    ...
    """
    for _contract in [proxy_contract, wrapped_impl]:
...
async def test_init_via_eic(
    ...
    exec_info = await proxy_contract.upgrade_to(
        impl1, eic_hash, eic_init_vector, NOT_FINAL
    ).execute(caller_address=gov)
    assert_events_equal(expected_event, exec_info.main_call_events[-1])
...
async def test_impl_revert(
    ...
    await proxy_contract.add_implementation(impl1, NO_EIC, [], NOT_FINAL).execute(
        caller_address=gov
    )
    await proxy_contract.add_implementation(impl2, NO_EIC, [], NOT_FINAL).execute(
        caller_address=gov
    )
    await proxy_contract.add_implementation(impl1, NO_EIC, init_vector1, NOT_FINAL).execute(
        caller_address=gov
...
async def test_upgrade_finalization(
    ...
    exec_info = await proxy_contract.upgrade_to(impl1, NO_EIC, [], FINAL).execute(
        caller_address=gov
    )
