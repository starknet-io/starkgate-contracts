...
mod permissioned_token_test {
    ...
    fn test_successful_permitted_mint() {
        ...
        let l2_token_address = deploy_l2_token(
            :initial_owner, :permitted_minter, initial_supply: u256 { low: 1000, high: 0 }
        );
        ...
        let minted_amount: u256 = u256 { low: 200, high: 0 };
        ...
        assert(
            erc20_token.balance_of(initial_owner) == u256 { low: 1200, high: 0 },
            'USED_ADDR_PERM_MINT_ERROR'
        );

        // Verify total supply.
        assert(
            erc20_token.total_supply() == u256 { low: 1400, high: 0 },
    ...
    fn test_overflowing_permitted_mint() {
        ...
        starknet::testing::set_contract_address(permitted_minter);
        let minted_amount: u256 = u256 { low: 1, high: 0 };
        let mint_recipient = starknet::contract_address_const::<1337>();
        mintable_token.permissioned_mint(account: mint_recipient, amount: minted_amount);
    }
    ...
    fn test_successful_permitted_burn() {
        ...
        starknet::testing::set_contract_address(permitted_minter);
        let burnt_amount: u256 = u256 { low: 200, high: 0 };

        // Burn from an address with existing balance.
        mintable_token.permissioned_burn(account: initial_owner, amount: burnt_amount);
        assert(
            erc20_token.balance_of(initial_owner) == u256 { low: 800, high: 0 },
            'USED_ADDR_PERM_BURN_ERROR'
        );

        // Verify total supply.
        assert(
            erc20_token.total_supply() == u256 { low: 800, high: 0 }, 'TOTAL_SUPPLY_PERM_BURN_ERROR'
        );
    }
    ...
    fn test_exceeding_amount_permitted_burn() {
        ...
        starknet::testing::set_contract_address(permitted_minter);
        let burnt_amount: u256 = u256 { low: 1001, high: 0 };
        mintable_token.permissioned_burn(account: initial_owner, amount: burnt_amount);
    }
