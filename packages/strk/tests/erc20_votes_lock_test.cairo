//! Tests for the ERC20VotesLock contract (vSTRK).
...
fn _erc20_votes_lock() -> ContractAddress {
    let locked_token = 20.try_into().unwrap();
...
fn votes_token_action(
    ...
    match action {
        ...
        VotesTokenFunction::Lock => {
            token_lock_interface
                .lock(:amount); // Event verification is implicit - balance changes are verified in
            // apply_action_and_verify
        },
        VotesTokenFunction::Unlock => {
            // Unlock the token.
            token_lock_interface.unlock(:amount);
            // Event verification is implicit - balance changes are verified in
        // apply_action_and_verify
        },
...
fn increase_allowance(erc20_token: ContractAddress, spender: ContractAddress, added_value: u256) {
    let erc20_lockable_interface = get_erc20_token(l2_token: erc20_token);
    erc20_lockable_interface.approve(:spender, amount: added_value);
}
...
fn test_invalid_caller_permissioned_lock_and_delegate() {
    ...
    let not_lockable_contract = 987.try_into().unwrap();
...
fn test_happy_flow_transfer_and_permissioned_and_delegate() {
    ...
    increase_allowance(
        erc20_token: lockable_token, spender: votes_lock_token, added_value: second_locked_amount,
    );
    // Caller lock and delegate to delgatee.
    lock_delegate_and_verify_total_supply_and_balance(
