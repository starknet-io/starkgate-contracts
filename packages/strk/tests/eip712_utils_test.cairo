//! Tests for the EIP712 utility functions.

use strk::eip712_utils::{
    calc_domain_hash, lock_and_delegate_message_hash, pedersen_hash_span, validate_signature,
};
use super::test_utils::deploy_account;
...
fn test_pedersen_hash_span() {
    ...
    let input_2 = array![1, 1, 2, 3, 5, 8].span();
    assert(
        pedersen_hash_span(
            elements: input_2,
...
fn validate_lock_and_delegate_hash(
    ...
    let account = 20.try_into().unwrap();
    let delegatee = 21.try_into().unwrap();
    let amount = 200;
    let nonce = 17;
    let expiry = 1234;
